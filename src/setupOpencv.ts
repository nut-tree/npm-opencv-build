...
function getRunBuildCmd(msbuildExe: string): () => Promise<void> {
  if (msbuildExe) {
    return async () => {
      await spawn(`${msbuildExe}`, getMsbuildCmd("./OpenCV.sln"), {
        cwd: dirs.opencvBuild
      });
      await spawn(`${msbuildExe}`, getMsbuildCmd("./INSTALL.vcxproj"), {
        cwd: dirs.opencvBuild
      });
    };
  }
  return async () => {
    await spawn("make", ["install", `-j${numberOfCoresAvailable()}`], {
      cwd: dirs.opencvBuild
    });
    // revert the strange archiving of libopencv.so going on with make install
    await spawn("make", ["all", `-j${numberOfCoresAvailable()}`], {
      cwd: dirs.opencvBuild
    });
  };
}
...
export async function setupOpencv() {
  ...
  log.info(
    "install",
    "installing opencv version %s into directory: %s",
    tag,
    dirs.opencvRoot
  );
  ...
  try {
    ...
    log.error(
      "install",
      "consider removing the folder yourself: %s",
      path.join(dirs.opencvRoot, "opencv")
    );
  }
  ...
  try {
    ...
    log.error(
      "install",
      "consider removing the folder yourself: %s",
      path.join(dirs.opencvRoot, "opencv_contrib")
    );
  }
}
